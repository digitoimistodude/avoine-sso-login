 ...
 * @Last Modified time: 2022-04-25 15:21:17
 *
...
function is_sso_user_active( $wp_user_id = null ) {
   ...
   */
  $active = apply_filters( 'avoine_sso_login\user\is_active', true, $wp_user_id, $ssoid, $sso_user_info );
  ...
  do_action( 'avoine_sso_login\user\is_active\after', $wp_user_id, $active );
  ...
  $expiration = apply_filters( 'avoine_sso_login\user\is_active\expiration', $expiration );
...
function create_wp_user( $sso_user = null ) {
  ...
  do_action( 'avoine_sso_login\user\create\before', $sso_user, $sso_user_info );
  ...
  $userdata['user_login'] = apply_filters( 'avoine_sso_login\user\create\user_login', $user_wp_unique, $sso_user, $sso_user_info );
  ...
  $userdata = apply_filters( 'avoine_sso_login\user\create', $userdata, $sso_user, $sso_user_info );
  ...
  do_action( 'avoine_sso_login\user\create\after', $new_user_id, $sso_user, $sso_user_info );
...
function get_user_data_for_wp( $sso_user ) {
  ...
  // if allowed using real email and it exists, use
  if ( apply_filters( 'avoine_sso_login\user\create\user_email\use_original', false ) && isset( $sso_user_info->{ $sso_user->idp . '.email_address' } ) && ! empty( $sso_user_info->{ $sso_user->idp . '.email_address' } ) ) {
  ...
  // use firstname in nicename if exists
  if ( isset( $userdata['first_name'] ) ) {
    $userdata['user_nicename'] = $userdata['first_name'];
  }

  // use last letter of lastname in nicename if exist
  if ( isset( $userdata['last_name'] ) ) {
    $userdata['user_nicename'] .= ' ' . $userdata['last_name'][0] . '.';
  }

  // allow developers to hook and modify user
  $userdata = apply_filters( 'avoine_sso_login\user\data', $userdata, $sso_user, $sso_user_info );

  return $userdata;
} // end update_wp_user

/**
 * Get the unique identified for SSO user.
 *
 * @since  2.0.0
 * @param  object $sso_user SSO user.
 * @return boolean[string   Unique identified for SSO user.
 */
function get_sso_user_mapping_id( $sso_user ) {
  ...
  $mapping_id = apply_filters( 'avoine_sso_login\user\mapping_id', $mapping_id, $sso_user, $sso_user_info );
